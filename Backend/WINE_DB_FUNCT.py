...
def get_varietals():
    varietals = conn.execute(postgres.text('SELECT column_name FROM information_schema.columns WHERE table_name = $$Wines$$'))
    varietals = [row[0] for row in varietals]
    varietals = varietals[3:]
    return varietals
...
def Wines_by_Winery_var_subset(winery_name=None, varietals_set=None):
    cond = []
